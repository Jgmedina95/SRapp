...
export regression_with_constraints, 
        regression_with_qbc, 
...
function regression_with_constraints(train_X, train_y, niterations, options1, options2, split; max_loops=nothing, target_error=nothing,convergence_jump = nothing )
    ...
    println("niterationconst2 is $(niterationconst2) \n\n ")
    println("niterationconst is $(niterationconst) \n\n ")
    ...
    while true
        ...
        try
            ...
            if max_qbc_iterations !== nothing && qbc_loop_count < max_qbc_iterations
                new_point, new_index = committee_evaluation(sample_X, dominating, options2; disagreement_measure=disagreement_measure)
